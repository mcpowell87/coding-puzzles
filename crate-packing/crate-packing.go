...
/*
 ...
 fit2():
 You upgrade your packing robot with the latest in packing technology: turning stuff. You now have the option of
 rotating all boxes by 90 degrees, so that you can treat a set of 6-by-5 boxes as a set of 5-by-6 boxes. You do
 not have the option of rotating some of the boxes but not others.
*/
...
func main() {
	...
	fmt.Println("Fit2")
	fmt.Println(fit2(25, 18, 6, 5))
	fmt.Println(fit2(12, 34, 5, 6))
	fmt.Println(fit2(12345, 678910, 1112, 1314))
	fmt.Println(fit2(5, 5, 3, 2))
	fmt.Println(fit2(5, 100, 6, 1))
	fmt.Println(fit2(5, 5, 6, 1))
}
...
// Returns the number of boxes with the specified size that will fit into the specified size crate
// Assuming that the crates can be rotated (not individually, all or nothing)
func fit2(crateX int, crateY int, boxesX int, boxesY int) float64 {
	return math.Max(fit1(crateX, crateY, boxesX, boxesY), fit1(crateX, crateY, boxesY, boxesX))
}
